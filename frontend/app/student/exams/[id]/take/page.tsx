      ...
      <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="mx-auto max-w-6xl px-4 flex h-16 items-center justify-between">
          ...
          <div className={`flex items-center gap-3 px-4 py-1.5 rounded-full border ${
            timeLeft !== null && timeLeft < 60 ? 'bg-rose-50 border-rose-200 animate-pulse' : 'bg-slate-50 border-slate-200'
          }`}>
            ...
            <span className={`text-lg font-black font-mono tracking-tighter ${
                 timeLeft !== null && timeLeft < 60 ? 'text-rose-600' : 'text-slate-700'
            }`}>
                ...
                <CardContent className="flex-1 p-8">
                    ...
                    >
                        {currentQuestion?.choices?.map((choice: any, index: number) => (
                           <label 
                            key={choice.id}
                            className={`flex items-center justify-between p-5 rounded-2xl border-2 transition-all cursor-pointer group hover:border-indigo-200 ${
                           ...
                           >
                                <div className="flex items-center gap-4">
                                    <div className={`h-8 w-8 rounded-full border-2 flex items-center justify-center font-bold text-sm transition-colors ${
                                        answers[currentQuestion?.id] === choice.id 
                                    ...
                                    <span className={`text-base font-semibold ${
                                         answers[currentQuestion?.id] === choice.id ? 'text-indigo-900' : 'text-slate-700'
                                    }`}>{choice.content}</span>
                                </div>
                         ...
                         {exam?.questions.map((_: any, idx: number) => (
                             <div 
                                ...
                                className={`h-1.5 w-1.5 rounded-full transition-all duration-300 ${
                                    idx === currentIndex ? 'w-6 bg-indigo-600' : answers[exam?.questions[idx]?.id] ? 'bg-indigo-300' : 'bg-slate-200'
                                }`}
                             />
                         ))}
                    </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {exam?.questions.map((q: any, idx: number) => (
                        <button
                            ...
                            className={`h-9 w-10 rounded-lg text-xs font-bold transition-all border ${
                                currentIndex === idx 
