    ...
    <div className="min-h-screen bg-slate-50/50 selection:bg-indigo-100 selection:text-indigo-900 font-sans">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-10">
        ...
        <div className="space-y-6">
          ...
          {/* Display joined classes logic here if available, using a simple card for now */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="border-slate-200 hover:border-indigo-400 transition-all cursor-pointer">
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-bold">Lớp 12-A1</CardTitle>
                <CardDescription>GV: Nguyễn Văn A</CardDescription>
              </CardHeader>
              <CardContent>
                <Badge className="bg-indigo-100 text-indigo-700">Đang hoạt động</Badge>
              </CardContent>
            </Card>
          </div>
        </div>
                    ...
                    submissions.map((sub) => (
                      <TableRow key={sub.id} className="border-slate-50 hover:bg-slate-50/50 transition-colors group">
                        ...
                        <TableCell className="text-right">
                          <div className={`text-lg font-black ${sub.score >= 8 ? "text-emerald-500" : sub.score >= 5 ? "text-amber-500" : "text-rose-500"
                            }`}>
      ...
      <footer className="mx-auto max-w-7xl px-4 py-12 border-t border-slate-200 mt-12 text-slate-400 text-sm font-medium">
        <p>© {new Date().getFullYear()} AzotaPro Learning System.</p>
      </footer>
    </div>
  );
}
